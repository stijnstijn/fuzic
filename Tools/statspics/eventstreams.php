...
define('CASTER_STREAMS', 10);
if(!defined('LHEIGHT')) {
    define('LHEIGHT', 40);
}

$x = 722;
$y = 24 + 125 + (LINE * CASTER_STREAMS);
...
write_text($img, 24, 0, 20, 80, $white, 'bold', $games[GAME]['name'].' top caster streams');
...
$ranking = $db->fetch_all("
    SELECT s.real_name, s.remote_id, s.avatar, s.player, r.stream, r.rank, r.average, r.peak, r.vh, r.time, r.delta
      FROM streams AS s,  ranking_month AS r
    ...
    if(!$p) {
        ...
        SELECT COUNT(*)
         ...
         WHERE r.game = ".$db->escape(GAME)."
           ...
           AND r.month = ".(MONTH == 1 ? 12 : MONTH - 1)."
           AND r.rank <= ".intval($p)."
           AND r.stream IN ( SELECT ".$db->escape_identifier(Models\Stream::IDFIELD)." FROM ".$db->escape_identifier(Models\Stream::TABLE)." WHERE player = 0 )");
        $ranking[$index]['delta'] = $index - $p + 1;
        $rank['delta'] = $index - $p + 1;
    }
    if ($rank['delta'] === NULL) {
        $delta_text = 'New!';
    } elseif ($rank['delta'] === 0) {
...
imagepng($img, '../../Site/assets/stats/'.GAME.'-'.YEAR.'-'.str_pad(MONTH, 2, '0', STR_PAD_LEFT).'-eventstreams.png');